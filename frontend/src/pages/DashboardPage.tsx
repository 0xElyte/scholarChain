...
export function DashboardPage({ wallet }: Props) {
  ...
  const activePools = MOCK_POOLS.filter((p) => p.state === "ACTIVE" || p.state === "PENDING");
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="scholar-card-strong rounded-3xl overflow-hidden mb-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
          ...
          <img src={DASHBOARD_PHOTO} alt="Students studying in a library" className="hidden md:block h-40 w-72 object-cover" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Total Pools",      value: MOCK_STATS.totalPools,                            sub: "across all creators",    color: "text-slate-800" },
          { label: "Active Right Now", value: MOCK_STATS.activePools,                           sub: "accepting proposals",    color: "text-teal-700" },
          { label: "Total Value",      value: `$${formatUSDT(MOCK_STATS.totalFunded)}`,         sub: "USDT deposited",         color: "text-amber-600" },
          { label: "Scholars Funded",  value: MOCK_STATS.totalWinners,                          sub: `$${formatUSDT(MOCK_STATS.totalGrantsClaimed)} claimed`, color: "text-emerald-600" },
        ...
        <section>
          ...
          {myCreated.length === 0 ? (
            <div className="bg-white rounded-xl border border-dashed border-teal-300 p-8 text-center shadow-sm">
              ...
              <Link to="/dashbar/create" className="text-sm font-semibold text-teal-700 hover:underline no-underline">Create your first pool →</Link>
            </div>
          ) : (
