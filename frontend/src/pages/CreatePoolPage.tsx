...
export function CreatePoolPage({ wallet }: Props) {
  ...
  function validate(): boolean {
    ...
    const valid = form.signers.filter((s) => /^0x[0-9a-fA-F]{40}$/.test(s.trim()));
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-8">
        ...
        <Fieldset title="Pool Identity">
          ...
          <Field label="Criteria Metadata CID" error={errors.criteriaMetadataCID} hint="Upload your criteria PDF to IPFS and paste the CID." required>
            <input
              type="text"
              placeholder="QmXyZ…"
        ...
        <Fieldset title="Funding">
          <Field label="USDT Token Address" error={errors.usdtTokenAddress} hint="The ERC-20 USDT contract on the target network." required>
          ...
          <Field label="Initial Deposit (USDT)" hint="Optional. A 10% protocol fee applies at distribution.">
        ...
        <div className="flex gap-3 p-4 rounded-xl bg-teal-50 border border-teal-200">
          ...
          <div className="text-sm text-teal-900">
            ...
            <p className="text-teal-700 mt-0.5">10% of total deposits go to the ScholarChain treasury at distribution. The remaining 90% goes to winners, or is refunded proportionally if there are none.</p>
          </div>
        </div>
      ...
      <Modal open={successModal} onClose={() => { setSuccess(false); navigate("/dashbar/explore"); }} title="Pool Deployed!">
        <div className="text-center py-4">
          <p className="text-5xl mb-4">🎉</p>
          <p className="text-slate-700 mb-1">
            Your pool <strong>{form.poolName}</strong> has been deployed.
          </p>
          <p className="text-sm text-slate-500 mb-6">Transaction submitted. It will appear in Explorer once confirmed.</p>
          <button
            onClick={() => { setSuccess(false); navigate("/dashbar/explore"); }}
