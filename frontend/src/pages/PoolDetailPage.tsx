...
export function PoolDetailPage({ wallet }: Props) {
  ...
  if (!pool) {
    return (
      <div className="max-w-lg mx-auto px-4 py-24 text-center">
        ...
        <h2 className="text-xl font-semibold text-slate-800 mb-2">Pool not found</h2>
        <p className="text-slate-500 text-sm mb-6">This address doesn't match any known pool.</p>
  ...
  const myProposal  = MOCK_PROPOSALS.find((p) => p.benefactor.toLowerCase() === addr);
  ...
  const canSubmit     = pool.state === "ACTIVE" && !myProposal && wallet.isConnected;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {(txPending || toast) && (
        <div className={`fixed bottom-6 right-6 z-50 px-5 py-3 rounded-xl shadow-lg text-sm font-medium ${
          txPending ? "bg-amber-100 text-amber-800 border border-amber-200" : "bg-emerald-100 text-emerald-800 border border-emerald-200"
        }`}>
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {canDonate && (
          <button onClick={() => setDonateModal(true)}
        ...
        {canDistribute && (
          <button onClick={() => stub("Distribution phase entered — 10% fee sent to treasury")} disabled={txPending}
        ...
        {canClaim && (
          <button onClick={() => stub("Grant claimed! Funds sent + SBT minted")} disabled={txPending}
        ...
        {canRefund && (
          <button onClick={() => stub("Refund claimed successfully")} disabled={txPending}
        ...
        {canCancel && (
          <button onClick={() => stub("Pool cancelled. Donors may claim refunds.")} disabled={txPending}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <Card title="Pool Overview">
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                ...
                ...(pool.distributionAmount !== "0" ? [{ l: "Per Winner", v: `${formatUSDT(pool.distributionAmount)} USDT`, c: "text-amber-600" }] : []),
                ...(pool.state === "ACTIVE" || pool.state === "PENDING" ? [{ l: "Time Remaining", v: timeRemaining(pool.submissionEnd), c: "text-teal-700" }] : []),
              ].map(({ l, v, c }) => (
              ...
              {pool.fieldDefinitions.map((f, i) => (
                <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-white border border-cyan-950/10 shadow-sm">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${
                    f.fieldType === FieldType.TEXT     ? "bg-slate-100 text-slate-600 border-slate-300" :
                    f.fieldType === FieldType.URL      ? "bg-teal-50 text-teal-700 border-teal-200" :
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-400 mt-0.5">
                            Submitted {formatDate(prop.submittedAt * 1000)} · CID: <span className="font-mono">{prop.documentCID.slice(0, 18)}…</span>
                          </p>
                        </div>
                        {prop.isWinner && (
                      ...
                      <div className="mb-3">
                        ...
                        <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${Math.min(100, (prop.approvalCount / quorum) * 100)}%` }}
                          />
                        </div>
                      </div>
                      ...
                      {canVote && !prop.isWinner && (
                        <div className="flex flex-wrap gap-2 mt-3">
                          {voteTarget === prop.benefactor ? (
                            <>
                              <button
                                onClick={() => { stub(`Voted Approve for ${shortAddr(prop.benefactor)}`); setVoteTarget(null); }}
                                disabled={txPending}
                              ...
                              <button
                                onClick={() => { stub(`Voted Reject for ${shortAddr(prop.benefactor)}`); setVoteTarget(null); }}
                                disabled={txPending}
          ...
          {myProposal && (
            <Card title="Your Proposal" highlight>
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  { l: "Document CID",    v: `${myProposal.documentCID.slice(0, 22)}…` },
                  { l: "Payout Address",  v: shortAddr(myProposal.payoutAddress) },
                  { l: "Approvals",       v: `${myProposal.approvalCount} / ${quorum}` },
                  { l: "Status",          v: myProposal.isWinner ? "🏆 Winner" : "⏳ Under Review" },
                ].map(({ l, v }) => (
        ...
        <div className="space-y-5">
          ...
          <Card title="Criteria Document">
            <p className="text-xs font-mono text-slate-600 break-all mb-2">{pool.criteriaMetadataCID}</p>
            <a href={`https://ipfs.io/ipfs/${pool.criteriaMetadataCID}`} target="_blank" rel="noreferrer"
              className="text-xs text-teal-700 hover:underline no-underline">View on IPFS ↗</a>
          </Card>

          <Card title="Contract">
            <p className="text-xs font-mono text-slate-600 break-all">{pool.address}</p>
          </Card>
        </div>
      </div>
      ...
      <Modal open={donateModal} onClose={() => setDonateModal(false)} title="Donate to Pool">
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            onClick={() => { setDonateModal(false); stub(`Donated ${donateAmount} USDT`); setDonateAmount(""); }}
      ...
      <Modal open={proposeModal} onClose={() => setProposeModal(false)} title="Submit Proposal">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" placeholder="IPFS CID of your application JSON"
            ...
            <p className="text-xs text-slate-400 mt-1">Upload your application to IPFS, paste the CID here.</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button
            onClick={() => { setProposeModal(false); stub("Proposal submitted successfully"); setDocCID(""); setPayoutAddr(""); }}
